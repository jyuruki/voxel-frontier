...
Voxel Frontier's **Highlands & Handshakes** release is Version 6.1. It is an original game inspired by voxel survival and factory-building genres; it does not claim one-to-one parity with Minecraft.

| System family | Highlands & Handshakes (Version 6.1) | Planned depth |
| --- | --- | --- |
...
| Multiplayer | One human-readable room code, encrypted automatic discovery, direct-first STUN, short-lived authenticated TURN fallback over UDP/TCP/TLS, reusable multi-peer rooms, host-owned WebRTC simulation, an 18-second transient-route grace period, validated combat/sleep/rift/drop state, snapshots, and manual signaling fallback | Permissions, private relay capacity, hosted room directory |
...
1. Stabilize Version 6.1 TURN-assisted rooms and tall-world performance across mobile browsers and restrictive networks.
2. Add farming, armor/equipment, weather, larger structures, breeding, and dedicated aquatic wildlife.
3. Add traversal vehicles, bosses, deeper dimension progression, permissions, and dedicated private relay capacity if public-room traffic grows.
